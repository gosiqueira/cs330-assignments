...
if __name__ == '__main__':
	...
	for ep in range(n_epochs):
		for epi in range(n_episodes):
			...
			batch_data, batch_labels = data_generator.sample_batch('meta_train', 1, shuffle=False, swap=False)

			# Support, query shape: N x K/Q x img width x img height x channels
			support = batch_data[:, :, :k_shot, :].reshape((n_way, k_shot, im_width, im_height, channels))
			query = batch_data[:, :, k_shot:, :].reshape((n_way, n_query, im_width, im_height, channels))
			labels = batch_labels[:, :, k_shot:, :].reshape((n_way, n_query, n_way))
			...
			if (epi+1) % 50 == 0:
				...
				batch_data, batch_labels = data_generator.sample_batch('meta_val', 1, shuffle=False, swap=False)
	...
	for epi in range(n_meta_test_episodes):
		...
		batch_data, batch_labels = data_generator.sample_batch('meta_test', 1, shuffle=False, swap=False)

        # support and query sets
		support = batch_data[:, :, :k_meta_test_shot, :].reshape((n_meta_test_way, k_meta_test_shot, im_width, im_height, channels))
		query = batch_data[:, :, k_meta_test_shot:, :].reshape((n_meta_test_way, n_meta_test_query, im_width, im_height, channels))
		labels = batch_labels[:, :, k_meta_test_shot:, :].reshape((n_meta_test_way, n_meta_test_query, n_meta_test_way))
